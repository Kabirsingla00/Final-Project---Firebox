...
if ( $args['featured'] == 'icon' && has_post_thumbnail() ) {
	$image_post = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), trx_addons_get_thumb_size('relimg') );
}

if (!empty($image_post)) {
	$image_bg = esc_attr(trx_addons_add_inline_css_class('background-image:url('.esc_url($image_post[0]).');'));	
} else{
	$image_bg = "";
}
		...
		if ( has_post_thumbnail() && $args['featured']=='image') {
			trx_addons_get_template_part('templates/tpl.featured.php',
											...
											apply_filters('trx_addons_filter_args_featured', array(
																...
																'hover' => 'zoomin',
																'no_links' => empty($link),
																'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size((int)$args['columns'] > 2 ? 'servimg' : 'big'), 'services-default'),
	...
	<div class="sc_services_item_info">
		...
		<?php if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) { ?>
			<div class="sc_services_item_content"><?php 
				if ($args['featured']=='icon' && (int)$args['no_padding']==0){
					echo wp_trim_words( get_the_content(), 7, '...' );
				} else if ($args['featured']=='icon' && (int)$args['no_padding']==1){

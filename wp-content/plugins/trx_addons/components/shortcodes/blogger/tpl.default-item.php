...
if ( has_post_thumbnail() ) {
	$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), trx_addons_get_thumb_size('relimg') );
}

if (!empty($image)) {
	$image_bg = esc_attr(trx_addons_add_inline_css_class('background-image:url('.esc_url($image[0]).');'));	
} else{
	$image_bg = "";
}

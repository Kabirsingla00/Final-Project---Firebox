...
if ( !function_exists( 'freightco_get_nav_menu' ) ) {
	function freightco_get_nav_menu($location='', $menu = '', $depth=11, $custom_walker=false) {
		...
		if (empty($list[$slug])) {
			$list[$slug] = esc_html__('You are trying to use a menu inserted in himself!', 'freightco');
			$args = array(
    ...
    function freightco_get_privacy_text() {
        ...
        return apply_filters( 'freightco_filter_privacy_text', wp_kses_post(
                ...
                . ( ! empty( $page ) && ! empty( $privacy_text )
                    ...
                    ? ' ' . sprintf( esc_html__( 'For further details on handling user data, see our %s', 'freightco' ),
                        '<a href="' . esc_url( get_permalink( $page ) ) . '" target="_blank">'
                        . esc_html__( 'Privacy Policy', 'freightco' )
                        . '</a>' )
                    : ''
                )
            )
...
/* wp_kses handlers
----------------------------------------------------------------------------------------------------- */
if ( ! function_exists( 'freightco_kses_allowed_html' ) ) {
	add_filter( 'wp_kses_allowed_html', 'freightco_kses_allowed_html', 10, 2);
	function freightco_kses_allowed_html($tags, $context) {
		if ( in_array( $context, array( 'freightco_kses_content' ) ) ) {
			$tags = array(
